...
function escribeRSS($Tipo){
	...
	$mysql = new mysql;
	$RSSpath = "../rss/";
	$SrcImg  = "http://www.sinaloahoy.com.mx/site/imagen.php?archivo=";
	...
	if($Tipo == "SINALOA"){
		$sql = "SELECT pub.IdPub AS IdPub, pub.Titulo AS Titulo, pub.Descripcion AS Descripcion, pub.Contenido AS Contenido, 
				...
				WHERE (pub.FechaPub <= CURDATE() AND pub.KeyGrupoHome = '1' AND pub.EstatusPub = 'A') ORDER BY IdPub DESC LIMIT 10";
		
        $Url     = "http://www.sinaloahoy.com.mx/site/publicacion.php?publicacion=";		
