...
try{
		...
		$rss = simplexml_load_file("http://www.noroeste.com.mx/rss/6.xml"); //LEO EL RSS
			if($rss){
				echo "Se ha leido el RSS";
				$rss->registerXPathNamespace('dc', "http://purl.org/dc/elements/1.1/"); //REGISTRO EL NAMESPACE PARA LA IMAGEN
				$i = 0;
				foreach ($rss->channel->item as $Datos){// LEO LAS PUBLICACIONES DE LOS RSS
					...
					$Param["TipoPub"]  = "PUB";
					$Param["User"] = "1";
					$Param["Titulo"]   =  utf8_decode((String)$Datos->title);
					$Param["Descripcion"] =  utf8_decode((String)$Datos->description);
					...
					$doc->loadHTML($url);
					$Param["Contenido"] = $doc->getElementById('tamano5')->nodeValue;
					//$Param["Contenido"] = htmlentities($Param["Contenido"]);
					$Param["Contenido"] = nl2br($Param["Contenido"]);
					$Param["Contenido"] = utf8_decode($Param["Contenido"]);
					
					echo $Param["Contenido"];
			...
			//				echo $pagina;
					...
					echo $retorno;
					*/
					if($i >= 1){ //Numero de publicaciones que va a leer el proceso
						break;	
					}
					$i++;
				}
