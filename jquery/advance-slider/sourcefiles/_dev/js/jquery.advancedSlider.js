		...
		function create() {
			...
			if (browserName == 'msie')
				if (parseInt(browserVersion) == 6)
					slider.addClass('ie6');
				else if (parseInt(browserVersion) == 7)
					slider.addClass('ie7');
				else if (parseInt(browserVersion) == 8)
					slider.addClass('ie8');
				else if (parseInt(browserVersion) == 9)
					slider.addClass('ie9');
			...
			if (slides.find('a.video').length) {
				slides.find('a.video').each(function() {
					...
					video.click(function(event) {
						...
						if (self.settings.leaveVideoAction == 'removeVideo' || isIOS || (browserName == 'msie' && parseInt(browserVersion) <= 7))
							currentVideo = iframe;
						else
							checkAndSetupVideo();
			...
			if (slider.find("a[rel^='slider-lightbox']").length)
				lightboxInstance = slider.find("a[rel^='slider-lightbox']").prettyPhoto({social_tools: '', 
																      deeplinking: false,
			...
			if (self.settings.disableRightClick) {
				slider.find('img').bind('contextmenu', function(e) {
					return false;
				});
		...
		*/
		function gotoSlide(index) {
			...
			if (isTransition || isImageLoading) {
				if (self.settings.overrideTransition) {
					switch(currentEffectType) {
						case 'slide':
							slideDivs[currentIndex].stop(false, true).css('visibility', '');
							break;
			...
			if (currentVideo) {
				...
				if ((self.settings.leaveVideoAction == 'removeVideo' || isIOS || (browserName == 'msie' && parseInt(browserVersion) <= 7)) && videoHiddenImage.length) {
					var videoHiddenImage= slideDivs[currentIndex].find('.video-hidden-image');
		...
		*/
		function completeTransition() {
			...
			slideDivs[currentIndex].children('.layer').not('.static, .init').each(function() {
				...
				if (browserName == 'msie' && parseInt(browserVersion) <= 7) {
					layer.css('visibility', 'inherit')
						 .css(target)
				} else {
					layer.addClass('init')
